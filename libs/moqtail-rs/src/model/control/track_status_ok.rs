...
impl ControlMessageTrait for TrackStatusOk {
  fn serialize(&self) -> Result<Bytes, ParseError> {
    let mut buf = BytesMut::new();
    buf.put_vi(ControlMessageType::TrackStatusOk)?;
    ...
    let payload_len: u16 = payload
      ...
      .map_err(|e: std::num::TryFromIntError| ParseError::CastingError {
        context: "TrackStatusOk::serialize(payload_length)",
        from_type: "usize",
        to_type: "u16",
        details: e.to_string(),
      })?;
  ...
  fn parse_payload(payload: &mut Bytes) -> Result<Box<Self>, ParseError> {
    ...
    if payload.remaining() < 1 {
      return Err(ParseError::NotEnoughBytes {
        context: "TrackStatusOk::parse_payload(group_order)",
        needed: 1,
    ...
    if let GroupOrder::Original = group_order {
      return Err(ParseError::ProtocolViolation {
        context: "TrackStatusOk::parse_payload(group_order_validation)",
        details: "Group order must be Ascending(0x01) or Descending(0x02)".to_string(),
      });
    }
    if payload.remaining() < 1 {
      return Err(ParseError::NotEnoughBytes {
        context: "TrackStatusOk::parse_payload(content_exists)",
        needed: 1,
        available: payload.remaining(),
      });
    }
    ...
    let param_count: usize =
      param_count_u64
        .try_into()
        .map_err(|e: std::num::TryFromIntError| ParseError::CastingError {
          context: "TrackStatusOk::parse_payload(param_count)",
          from_type: "u64",
          to_type: "usize",
          details: e.to_string(),
        })?;
...
mod tests {
  ...
  fn test_roundtrip() {
    ...
    let track_status_ok = TrackStatusOk {
      request_id,
      track_alias,
      expires,
      group_order,
    ...
    let mut buf = track_status_ok.serialize().unwrap();
    let msg_type = buf.get_vi().unwrap();
    assert_eq!(msg_type, ControlMessageType::TrackStatusOk as u64);
    let msg_length = buf.get_u16();
    assert_eq!(msg_length as usize, buf.remaining());
    let deserialized = TrackStatusOk::parse_payload(&mut buf).unwrap();
    assert_eq!(*deserialized, track_status_ok);
  ...
  fn test_excess_roundtrip() {
    ...
    let serialized = track_status_ok.serialize().unwrap();
  ...
  fn test_partial_message() {
    ...
    let deserialized = TrackStatusOk::parse_payload(&mut partial);
