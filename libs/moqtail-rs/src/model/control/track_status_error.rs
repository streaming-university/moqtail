...
impl ControlMessageTrait for TrackStatusError {
  fn serialize(&self) -> Result<Bytes, ParseError> {
    let mut buf = BytesMut::new();
    buf.put_vi(ControlMessageType::TrackStatusError)?;
...
mod tests {
  ...
  fn test_roundtrip() {
    ...
    assert_eq!(msg_type, ControlMessageType::TrackStatusError as u64);
    let msg_length = buf.get_u16();
    assert_eq!(msg_length as usize, buf.remaining());
    let deserialized = TrackStatusError::parse_payload(&mut buf).unwrap();
  ...
  fn test_partial_message() {
    ...
    let deserialized = TrackStatusError::parse_payload(&mut partial);
